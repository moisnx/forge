...
if (typeof morphdom === "undefined" && typeof window !== "undefined") {
  window.morphdom = window.morphdom || this.morphdom;
}

(function () {
  "use strict";
  // We need to connect to the WebSocket server started by the dev server
  // But we don't want to hardcode the URL, so we use the same port as the server
  const WS_URL = "{{ livereload_ws_url }}";
  let ws;
    ...
    ws.onmessage = async (event) => {
      try {
        ...
        if (
          ...
          // Don't morph stylesheets - exclude them from morphdom
          morphdom(document.head, newDoc.head, {
            onBeforeElUpdated: function (fromEl, toEl) {
              // Don't update stylesheet links
              if (fromEl.tagName === "LINK" && fromEl.rel === "stylesheet") {
                return false; // Skip updating stylesheets
              }
              return true;
            },
          });
          ...
          if (window._cssReloading) return;
          window._cssReloading = true;
          ...
          links.forEach((link) => {
            // Just update the href directly with cache buster
          ...
          setTimeout(() => {
            window._cssReloading = false;
          }, 100);
        } else {
          window.location.reload();
        }
      } catch (e) {
        console.error("[LiveReload] Error:", e);
      }
    };
